...
public:
	...
	void RecordData(const FBareItem& Item, int32 TopLeft);

	bool IsRoomAvailable(const FBareItem& Item, int TopLeftIndex);

	int32 GetFirstValidTopLeft(const FBareItem& Item);
private:
	int32 Width = 1;
	int32 Height = 1;
	TArray<const FBareItem*> Grid;
};
...
class INVENTORYPOC_API UBagStorage : public UActorComponent
{
	GENERATED_BODY()
...
protected:
	...
	BagSlot LocalBagSlot = BagSlot::Unknown;
...
public:
	...
	bool InitializeData(BagSlot InputBagSlot, int32 InputWidth, int32 InputHeight,
	...
	void AddItemAt(int64 ItemID, int32 TopLeftIndex);
	...
	UFUNCTION(BlueprintCallable, Category = "Inventory|Bag")
	BagSlot GetBagSlot() const { return LocalBagSlot; }

	UFUNCTION(BlueprintCallable, Category = "Inventory|Bag")
	void SetBagSlot(BagSlot InputBagSlot) { LocalBagSlot = InputBagSlot; }
